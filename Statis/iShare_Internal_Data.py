...
ishare_db=MySQLdb.connect(host='172.16.15.103',port=3306,user='uroot', passwd='psoft', db='isharedata',charset='utf8')
#ishare_db=MySQLdb.connect(host='localhost',port=3306,user='ishare', passwd='1qaz6yhn', db='isharedata',charset='utf8')
ishare_cursor = ishare_db.cursor()
...
print ''
print ''            
print "iShare  internal network Statistics " +Start_Date + "---" + End_Date
...
print ' '
print ' '
print "Department_Name_Level_1" + " "*(DEP_NAME_LEN-len("Department_Name_Level_1")) + "\t\tAll_User\t\tArticles\t\tUsage_Rate"
print '' 
...
for a_dep in All_Dep.keys():
    ...
    for  EName,DepID1,DepID2  in user_cursor:
        ...
        for a_day in All_Day:
            ...
            task_sql_1 =   "select ID  FROM sendrele  where datetime>='" + a_day + " 00:00:00' and datetime<='" + a_day + " 23:59:59' and " 
            task_sql_1 =  task_sql_1 + " username='" + USER_NAME + "'"
            
            records=0
            ishare_cursor.execute(task_sql_1)
            for ID in ishare_cursor:
                if ID[0] not in All_iShare_ID:
                    All_iShare_ID.append(ID[0])
                if records==0:
    ...
    DEP_1_Content.update({a_dep:len(All_iShare_ID)})
    print a_dep + " "*(DEP_NAME_LEN-len(a_dep)) + "\t\t" + str(all_user) + "\t\t" + str(len(All_iShare_ID)) + "\t\t" + parten.sub('',str(round(all_fill/all_user,4)*100)) 
...
print ' '
print ' '
print u"Department_Name_Level_1" + " "*(DEP_NAME_LEN-len("Department_Name_Level_1")) + "\t\tAll_User\t\tArticles\tvUsage_Rate"
print ''
...
for a_dep in All_Dep.keys():
    ...
    for a_dep2 in All_Dep[a_dep].keys():
        ...
        for  EName,DepID1,DepID2  in user_cursor:
            ...
            all_fill_per = all_fill_per + (fill_count+ 0.00)/len(All_Day)
        tmp = a_dep + " | " + a_dep2 
        DEP_2_Content.update({tmp:len(All_iShare_ID)})
        print tmp + " "*(DEP_NAME_LEN-len(tmp)) + "\t\t" + str(all_user) + "\t\t" + str(len(All_iShare_ID)) + "\t\t" + parten.sub('',str(round(all_fill/all_user,4)*100))
...
print ''
print ''
print u"Department_Name_Level_3" + " "*(DEP_NAME_LEN-len("Department_Name_Level_3")) + "\t\tAll_User\t\tArticles\t\tUsage_Rate"
print ''
